...
$(function () {
  ...
  function cal_deleted_img() {
    ...
    if (del_leng > 0) {
      ...
      else $(".select-msg-text-block").html(del_leng + " images selected");
  ...
  function update_video_links() {
    if ($(".vdo_url_container").find(".form-group").length <= 1) {
      $(".vdo_url_container")
        .find(".form-group .remove-video-link")
        .addClass("disabled");
    } else {
      $(".vdo_url_container")
        .find(".form-group .remove-video-link")
  ...
  function update_video_sections(parents) {
    if (parents.find(".video-url-container").length <= 1) {
      parents
        .find(".video-url-container .rmv-vdo-url-btn")
        .addClass("disabled")
        .attr("disabled", true);
    } else {
      parents
    ...
    parents.find(".video-url-container").each(function (i) {
      var row_count = i + 1;
      ...
      if ($(this).find("label").length) {
        $(this)
          ...
          .attr(
            "for",
            $(this).find("label").attr("for").replace(/\d+/, row_count)
          );
      }
  ...
  function update_site_language_name_id_func() {
    ...
    $(".lang-container .single-lang-block").each(function (i) {
        ...
        .each(function () {
          if ($(this).find("select").length) {
            $(this)
              ...
              .attr(
                "id",
                $(this).find("select").attr("id").replace(/\d+/, row_count)
              );
            $(this)
              .find("select")
              .attr(
                "name",
                $(this).find("select").attr("name").replace(/\d+/, row_count)
              );
          }
          if ($(this).find('input[type="radio"]').length) {
            $(this)
              .find('.radio_toggle_wrapper input[type="radio"]')
              .each(function () {
                $(this).attr(
                  "id",
                  $(this).attr("id").replace(/\d+/, row_count)
                );
                $(this).attr(
                  "name",
                  $(this).attr("name").replace(/\d+/, row_count)
                );
              });
          }
          if ($(this).find("label").length) {
            if ($(this).find("label").length == 1)
              ...
              $(this)
                .find(".radio_toggle_wrapper label")
                .each(function () {
                  $(this).attr(
                    "for",
                    $(this).attr("for").replace(/\d+/, row_count)
                  );
                });
            }
          }
      ...
      $(this)
        .find('.radio_toggle_wrapper input[type="radio"]:checked')
        .next("label")
        .trigger("click");
  ...
  if ($(".homepage_section_form").length) {
    if ($(".show_hide_block_btn").length) {
      $(".show_hide_block_btn").each(function (e) {
        /*:not(".child_element_block")*/ var thiss = $(this);
        var li_parents = thiss.parents("li");
        var elem_name = $(this).attr("name");
        var elem_val = li_parents
          .find("input[name='" + elem_name + "']:checked")
          .val();
        var data_target = li_parents
          .find("input[name='" + elem_name + "']:checked")
          .attr("data-target");
        li_parents.find("." + data_target).hide();
        li_parents
          .find("." + data_target + "." + elem_val + "_block")
          .addClass("hidden-elem")
          .show();
      });

      $(".show_hide_block_btn").each(function (e) {
        if (
          !$(this).parents(".form-group").hasClass("carousel_block") &&
    ...
    $('.show_hide_block_btn:not(".child_element_block")').change(function () {
      ...
      var data_target = $("input[name='" + elem_name + "']:checked").attr(
        "data-target"
      );
      $("." + data_target).hide();
    ...
    $(".add-video-url-btn").on("click", function () {
      var thiss = $(this);
      var parents = thiss.parents(".section_fields");
      var cloned_elem = thiss
        .parents("li")
        .find(".video-url-container")
        .eq(0)
        .clone(true);
      cloned_elem.find('input[type="url"]').val("");
      thiss.parents(".form-group").before(cloned_elem);
      update_video_sections(parents);
      return false;
    });

    $(".rmv-vdo-url-btn").on("click", function () {
      var thiss = $(this);
      var parents = thiss.parents(".section_fields");
      if (confirm("Do you really want to delete this video URL?")) {
        thiss.parents(".video-url-container").remove();
        update_video_sections(parents);
      }
      return false;
    });

    if ($(".video-url-container").length) {
      $(".video-url-container").each(function () {
        var thiss = $(this);
        var parents = thiss.parents(".section_fields");
        update_video_sections(parents);
      });
  ...
  if ($(".add_property_form").length) {
    ...
    $(".add_more_vdo_btn").on("click", function () {
      var cloned_elem = $(".vdo_url_container")
        .find(".form-group")
        .eq(0)
        .clone(true);
      ...
      $(".vdo_url_container .form-group").each(function (i) {
        ...
        $(this)
          ...
          .attr(
            "id",
            $(this).find(".video_url").attr("id").replace(/\d+/, row_count)
          );
        $(this)
          .find("label")
          .attr(
    ...
    $(".custom-dropdown-menu li a").on("click", function () {
      ...
      $(this)
        ...
        .html(f_value + '&nbsp;&nbsp;<span class="fa fa-caret-down"></span>');
      return false;
    });
    ...
    $(document).delegate(".remove-product-btn", "click", function () {
      ...
      if (img_ids != "") {
        ...
        $('input[type="hidden"][name="addedImgFromMediaLibrary"]').val(
          ids_array.join(",")
        );
      }
      return false;
    ...
    $(".add_from_media_btn").magnificPopup({
      ...
      ajax: {
        settings: {
          url: base_url + "admin_ajax?callback=add_image_from_media_ajax",
          type: "POST",
        },
      ...
      callbacks: {
        ...
        ajaxContentAdded: function () {
          $(".media_img_block .lazy-img-elem").lazy({
            effect: "fadeIn",
            effectTime: 500,
            threshold: 400,
            afterLoad: function (element) {
              element.parent().removeClass("lazy-load-processing");
              element.removeClass("lazy-img-elem");
            },
          });
        },
      },
    ...
    $(document).delegate(".insert_in_product", "click", function () {
      ...
      $("ul.media_img_block li").each(function () {
        if ($(this).find("a img").hasClass("selected")) {
          ...
          var img =
            "<div class='media-img-block'><div class='media_images_inner' data-container='body' data-toggle='tooltip' title='" +
            img_title +
            "'><span class='remove-product-btn'><i class='fa fa-remove'></i></span><img data-img_id='" +
            $(this).find("a").attr("data-image-id") +
            "' src=";
          img += img_src;
          img += "></div></div>";
          data += img;
      ...
      $(".product-gallary-container").sortable({
        stop: function (event, ui) {
          ...
          $(".product-gallary-container .media-img-block").each(function () {
            ...
            if (img_id != "") imgEditArray.push(img_id);
          });
          $('input[type="hidden"][name="addedImgFromMediaLibrary"]').val(
            imgEditArray.join(",")
          );
  ...
  $(".keywords").on("change", function () {
    ...
    var value = thiss.val();
    var callback = "update_site_lang_keywords";
    $.ajax({
      ...
      success: function (res) {
        ...
        thiss
          .parent()
          .append('<span class="label label-success" >Updated</span>');
...
if($(".property_for_states").length){
  ...
  $(".featured-prod-checkbox").on("ifChanged", function (event) {
  ...
  $(".featured-prod-checkbox").on("change", function () {
    ...
    var is_feat = "N";
    if (thiss.is(":checked")) is_feat = "Y";
    var callback = "toggle_featured_property_callback_func";
    $.ajax({
      url: base_url + "admin_ajax",
      type: "POST",
      success: function (res) {
      ...
      data: { is_feat: is_feat, p_id: p_id, callback, callback },
      cache: false,
    });
...
if($(".user_types").length){
  ...
  $("#UserName").on("keyup", function () {
    ...
    var thiss = $(this);
    if (user_name != "") {
		var callback = "check_username_existence";
      $.ajax({
        ...
        data: { user_name: user_name , callback : callback },
        cache: false,
      });
    }
    return false;
  ...
  $(document).delegate("a.remove_album_image", "click", function () {
    ...
    /*$('.full_sreeen_overlay').show();*/
    var callback = "delete_gallery_images";
    $.ajax({
      url: base_url + "admin_ajax",
      type: "POST",
      success: function (res) {
      ...
      data: { img_name: img_name, image_type: image_type, callback: callback },
      cache: false,
    });
    return false;
  });
  ...
  if ($(".site_language_form").length) {
    ...
    $(document).delegate(".remove-lang-block", "click", function () {
      if (
        $(this)
          .parents(".single-lang-block")
          .find('input[type="radio"][name="options[default_language]"]')
          .is(":checked")
      ) {
        $(this)
          .parents(".lang-container")
          .find(".single-lang-block")
          .eq(0)
          .find('input[type="radio"][name="options[default_language]"]')
          .iCheck("check");
      }
    ...
    $('input[type="radio"][name="options[default_language]"]').on(
      "ifChanged",
      function () {
        ...
        thiss
          .parents(".single-lang-block")
          .find(".remove-lang-block")
          .addClass("disabled");
      }
    );

    $(".site_language_form").on("submit", function () {
      $('input[type="radio"][name="options[default_language]"]:checked')
        .parents(".single-lang-block")
        .find('input[type="radio"][value="enable"]')
        .trigger("click")
        .trigger("change");
    });
  }
  ...
  $(".form").on("submit", function (e) {
    $(".form")
      ...
      .filter(function (index, element) {
        if (
          !$(element).hasClass("distances_list") &&
          !$(element).hasClass("amenities_list") &&
          !$(element).hasClass("property_for_cities") &&
          !$(element).hasClass("property_for_states") &&
          !$(element).hasClass("no_clean") &&
          !$(element).hasClass("assign_to_list")
        ) {
          var updated_string = escapeHtml($(element).val());
          $(element).val(updated_string);
        }
  ...
  $(
    'input[type="checkbox"].minimal-green, input[type="radio"].minimal-green'
  ).iCheck({
  ...
  $(".control-sidebar .tab-pane .scrollable_tab").slimscroll({
    height:
      $(window).height() -
      ($(".main-header").height() +
        $(".control-sidebar ul.nav-tabs").height() +
        $(".control-sidebar .tab-content h3.control-sidebar-heading").height() *
          4) +
      5 +
      "px",
    color: "rgba(0,0,0,0.2)",
  ...
  $(".remove_property_notif").on("click", function () {
    if (confirm("Do you really want to delete this notification?")) {
      ...
      $.ajax({
        ...
        success: function (res) {
          if (res == "success") {
            thiss.parents("li").fadeOut(300, function () {
              thiss.parent().remove();
            });
          }
        },
  ...
  if ($(".show_hide_setting_elem").length) {
    $(".show_hide_setting_elem").each(function () {
      ...
      $("." + show_hide_elem).hide();
      var target = $("input[name='" + elem_name + "']:checked").attr(
        "data-target"
      );
      $("." + target).show();
    });
      ...
      open: function () {
        ...
        if (google_map_api_key != "") {
          $.getScript(
            ...
            function () {
              initialize(
                parseFloat(google_map_center_latitude),
                parseFloat(google_map_center_longitude)
              );
            }
          );
        } else {
  ...
  $(".popup-property").magnificPopup({
    ...
    callbacks: {
      ...
      open: function () {
        ...
        if (map_lat == "") map_lat = -34.397;
        if (map_lng == "") map_lng = 150.644;
