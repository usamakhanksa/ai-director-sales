                                            ...
                                            <div class="row">
                                                    ...
                                                    <div class="form-inner">
                                                        ...
                                                        <select  class="select-template" name="template_access[]" multiple="multiple">
                                                            ...
                                                            @if(old("template_access") && is_array(old("template_access")))
                                                               @foreach (App\Models\AiTemplate::with('category')->text()->default()->cursor() as $template )
                                                ...
                                                <div class="col-sm-4">
                                                    <div class="form-inner mb-0">
                                                        ...
                                                        <input type="number" min="-1" value="{{old('ai_configuration.word_limit')}}" name="ai_configuration[word_limit]" id="word_limit" placeholder="{{translate('No. of Words')}}">
                                                    </div>
                                                </div>
                                                <!-- Image AI Model -->
                                                <div class="col-sm-4 mt-3">
                                                    <div class="form-inner mb-0">
                                                        <label for="image_ai_model">
                                                            {{translate("Image AI Model")}}
                                                        </label>
                                                        <select class="ai-select2 form-select" id="image_ai_model" name="ai_configuration[image_ai_model]">
                                                            <option value="">
                                                                {{translate("Select Image Model")}}
                                                            </option>
                                                            @foreach (Arr::get(config('settings'), 'ai_image_generation_model', []) as $k => $v)
                                                                <option value="{{$k}}" {{old("ai_configuration.image_ai_model") == $k ? "selected" : ""}}>
                                                                    {{ $v }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="col-lg-4 mt-3">
                                                    <div class="form-inner">
                                                        <label for="image_ai_model">
                                                            {{translate("Image Template Access")}}
                                                        </label>
                                                        <select class="select-image-template" name="image_template_access[]" multiple="multiple">
                                                            <option value="">
                                                                {{translate('Select Image Template')}}
                                                            </option>
                                                            @if(old("image_template_access") && is_array(old("image_template_access")))
                                                                @foreach (App\Models\AiTemplate::default()->image()->cursor() as $template)
                                                                    <option selected value="{{$template->id}}">
                                                                        {{$template->name}}
                                                                    </option>
                                                                @endforeach
                                                ...
                                                <div class="col-sm-4 mt-3">
                                                    <div class="form-inner mb-0">
                                                        <label for="image_limit">{{ translate('No. Of Images') }}
                                                            <small class="text-danger">*</small>
                                                            <i data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{translate('Set -1 to make it unlimited')}}" class="ms-1 pointer las la-question-circle text--danger"></i>
                                                        </label>
                                                        <input type="number" min="-1" value="{{old('ai_configuration.image_limit')}}" name="ai_configuration[image_limit]" id="image_limit" placeholder="{{translate('No. of Images')}}">
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-sm-4 mt-3">
                                                    <div class="form-inner mb-0">
                                                        <label for="image_ai_model">
                                                            {{translate("Video AI Model")}}
                                                        </label>
                                                        <select class="ai-select2 form-select" id="video_ai_model" name="ai_configuration[video_ai_model]">
                                                            <option value="">
                                                                {{translate("Select Video Model")}}
                                                            </option>
                                                            @foreach (Arr::get(config('settings'), 'ai_video_generation_model', []) as $k => $v)
                                                                <option value="{{$k}}" {{old("ai_configuration.video_ai_model") == $k ? "selected" : ""}}>
                                                                    {{ $v }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="col-lg-4 mt-3">
                                                    <div class="form-inner">
                                                        <label for="image_ai_model">
                                                            {{translate("Video Template Access")}}
                                                        </label>
                                                        <select class="select-video-template" name="video_template_access[]" multiple="multiple">
                                                            <option value="">
                                                                {{translate('Select Video Template')}}
                                                            </option>
                                                            @if(old("video_template_access") && is_array(old("video_template_access")))
                                                                @foreach (App\Models\AiTemplate::default()->video()->cursor() as $template)
                                                                    <option selected value="{{$template->id}}">
                                                                        {{$template->name}}
                                                                    </option>
                                                                @endforeach
                                                ...
                                                <div class="col-sm-4 mt-3">
                                                    <div class="form-inner mb-0">
                                                        <label for="video_limit">{{ translate('No. Of Videos') }}
                                                            <small class="text-danger">*</small>
                                                            <i data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{translate('Set -1 to make it unlimited')}}" class="ms-1 pointer las la-question-circle text--danger"></i>
                                                        </label>
                                                        <input type="number" min="-1" value="{{old('ai_configuration.video_limit')}}" name="ai_configuration[video_limit]" id="video_limit" placeholder="{{translate('No. of Videos')}}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script nonce="{{ csp_nonce() }}">
	(function($){
           ...
           $(".select2").select2({
            ...
            $('.select-image-template').select2({

                placeholder: "{{ translate('Select Template') }}",
                allowClear: false,
                tags: true,
                ajax: {
                    url: "{{ route('admin.subscription.package.selectSearch.image') }}",
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
            ...
            $('.select-video-template').select2({

                placeholder: "{{ translate('Select Template') }}",
                allowClear: false,
                tags: true,
                ajax: {
                    url: "{{ route('admin.subscription.package.selectSearch.video') }}",
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
